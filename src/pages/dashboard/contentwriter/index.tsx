...
import { countWords, trimString } from "@/utils/misc";
...
const Contentwriter = () => {
  ...
  const rewriteContent = async () => {
    ...
    const { content, keywords, language } = formData;
    const reqBody = {
      content,
      keywords,
      wordcount: 150,
    ...
    try {
      ...
      // Record log
      await axiosInstance.post("/ai/userhistory/record-log", {
        userId: user._id,
        templateName: "Content Rewrite",
        userInputData: trimString(content, 50),
        language,
      });
      // Update Usage
      await axiosInstance.post("ai/subscription/update-usage", {
        userId: user._id,
